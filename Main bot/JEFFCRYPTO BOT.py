...
# Load ADMIN_IDS from file if it exists
...
async def check_password(update: Update, context: CallbackContext) -> int:
    ...
    if password_attempt == BOT_PASSWORD or user_id in ADMIN_IDS:
        ...
        # Dynamically add the user to ADMIN_IDS if they are not already an admin
        if user_id not in ADMIN_IDS:
            ADMIN_IDS.append(user_id)
            logger.info(f"User {user_id} added to ADMIN_IDS.")

            # Save the updated ADMIN_IDS list to a file
...
async def disable_selling_duration(update: Update, context: CallbackContext) -> int:
    ...
    try:
        ...
        # Disable selling via custom smart contract
        deep_link = await solana_utils.disable_selling(PublicKey(mint_address), PublicKey(phantom_wallet_address), days)
        if not deep_link:
            await update.message.reply_text("Failed to disable selling. Please try again.")
            return ConversationHandler.END

        # Notify the user
        await update.message.reply_text(
            f"✅ Selling is disabled for {days} days.\n"
            f"Token {mint_address} is now protected from selling."
        )
...
async def copy_token_disable_selling(update: Update, context: CallbackContext) -> int:
    """Handle disable selling duration input for copying a token."""
    try:
        days = int(update.message.text)
        if days < 1 or days > 7:
            await update.message.reply_text("Duration must be between 1 and 7 days.")
...
def main() -> None:
    ...
    copy_conv_handler = ConversationHandler(
        ...
        states={
            ...
            DISABLE_SELLING: [MessageHandler(filters.TEXT & ~filters.COMMAND, copy_token_disable_selling)],
            REVOKE_AUTHORITIES: [MessageHandler(filters.TEXT & ~filters.COMMAND, revoke_authorities_prompt)],
        },
        fallbacks=[],
    )
